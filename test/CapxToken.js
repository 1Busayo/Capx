   ...
   return tokenInstance.standard();
	...
	it('allocates the initial supply upon deployment', function(){
		return CapxToken.deployed().then(function(instance){
			...
			assert.equal(totalSupply.toNumber(), 2000000,'sets the total supply to 2,000,000');
		return  tokenInstance.balanceOf(accounts[0]);
		}).then(function(adminBalance){
     assert.equal(adminBalance.toNumber(), 2000000 , 'it allocates the initial supply to the admin')
		});
		});
		...
		assert(error.message, 'error message must contain revert');
			  ...
			  assert.equal(balance.toNumber(), 1750000, 'deducts the amount from the sending account');
			});
		  });
...
// Aprove spendingAccount to spend 10 tokens form fromAccount
